...
class Day1_03_Flatten{
	...
	public static void main(String args[]) throws Exception{
		System.setIn(new FileInputStream("Ipt (Flatten).txt"));
	...
	public static void Flatten(int Fst, int Lst) {
	//	종료 조건
		if(N == 0)	{
			return;
		}
	...
	//	기준점 정렬
		for(int i = 0; i < 99; i++) {
			if(Box[i] == Box[i + 1])	Fst++;	//	최솟값이 2개면 기준점을 옮김.	
			else	break;
		}
		
		for(int i = 99; 1 < i; i--) {
			if(Box[i] == Box[i - 1])	Lst--;	//	최댓값이 2개면 기준점을 옮김.	
			else	break;
		}
		
	//	연산
		int tmp = 0;
		for(int i = 99; Lst <= i; i--) {
			Box[i]--;
			N--;
			tmp++;
			if(N == 0)	break;	//	중간에 덤프 가능 횟수 Deg가 0이 되면 차감 종료
		}
		...
		for(int i = 0; i <= Fst; i++) {
			...
			if(tmp == 0)	break;	//	중간에 덤프된 횟수 tmp가 0이 되면 가산 종료
			Box[i]++;
			tmp--;
		}	
